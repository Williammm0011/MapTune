...
class GateSelectionEnv(gym.Env):
    ...
    def __init__(self, genlib_origin, lib_path, design, total_gates, sample_gate, max_delay, max_area):
        ...
        self.action_space = spaces.Discrete(
            self.total_gates)  # Select one gate at a time
        self.observation_space = spaces.MultiBinary(
            self.total_gates)  # Binary flags for each gate
    ...
    def step(self, action):
        ...
        if done:
            ...
            delay, area = self.technology_mapper(
                list(np.where(self.state == 1)[0]))
    ...
    def technology_mapper(self, partial_cell_library):
        ...
        output_genlib_file = self.lib_path + self.design + "_" + \
            str(len(lines_partial)) + "_ddqn_samplelib.genlib"
...
class DDQNAgent:
    def __init__(self, state_size, action_size, learning_rate=0.001, tau=0.01):
        ...
        self.online_network = DDQNNetwork(
...
abc_cmd = "read %s;read %s; map -a; write %s; read %s;read -m %s; ps; topo; upsize; dnsize; stime; " % (
...
env = GateSelectionEnv(genlib_origin, lib_path, design,
                       total_gates, sample_gate, max_delay, max_area)
