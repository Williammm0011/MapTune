...
def main():
    ...
    temp_blif = args.temp_blif if os.path.isabs(
        args.temp_blif) else os.path.join(_ROOT, args.temp_blif)
    ...
    logger.info(
        f"{'Iter':>5} | {'Temp':>6} | {'Gates':>5} | {'Area':>8} | {'Delay':>8} | {'Cost':>8} | {'Action'}")
    logger.info("-" * 72)
    ...
    for step in range(1, args.iterations + 1):
        ...
        if len(neighbor_state) == 0:
            ...
            n_area, n_delay = mapper.map_subset(
                sorted(list(neighbor_state)), tag=f"sa_{step}")
        ...
        if step % 100 == 0 or step == 1:
            ...
            logger.info(
                f"{step:5d} | {T:6.4f} | {len(current_state):5d} | {curr_area:8.2f} | {curr_delay:8.2f} | {current_cost:8.4f} | {acc_str} ({action_str})")

    logger.info("\n" + "=" * 72)
    logger.info("Optimization Complete!")
    logger.info(f"Elapsed Time : {time.time() - start_time:.1f} seconds")
    logger.info(
        f"Baseline ADP : 1.0000 (Area: {base_area:.2f}, Delay: {base_delay:.2f})")
    logger.info(
        f"Best SA Cost : {best_cost:.4f} (Area: {best_area:.2f}, Delay: {best_delay:.2f})")
